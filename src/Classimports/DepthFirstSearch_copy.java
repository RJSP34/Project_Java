 ...
 *  @author Kevin Wayne
 */
public class DepthFirstSearch_copy {
     ...
     */
    public DepthFirstSearch_copy(EdgeWeightedDigraph_copy G , int s) {
    ...
    private void dfs(EdgeWeightedDigraph_copy G, int v) {
        count++;
        marked[v] = true;
        for (DirectedEdge_copy w : G.edges()) {
            if (!marked[w.to()]) {
                dfs(G, w.to());
            }
        }
    ...
    public static void main(String[] args) {
        ...
        EdgeWeightedDigraph_copy G  = new EdgeWeightedDigraph_copy(6);
        DepthFirstSearch_copy search = new DepthFirstSearch_copy(G, 3);
        for (int v = 0; v < G.V(); v++) {
            if (search.marked(v))
                StdOut.print(v + " ");
        }
